...
function guessModel(data, tablesOptions = {}, sequlizeInstance) {
  ...
  let dbOptions = tablesOptions.dbOptions ?? {};
  ...
  let modelOptions = {
    ...
    tableName: dbUtils.sqlName(modelName, 'table', dbOptions.forceSnakeCase)
  };

  // Go through each column
  _.each(data[0], (v, columnName) => {
    let fieldName = _.camelCase(columnName);
    let sqlName = dbUtils.sqlName(columnName, 'col', dbOptions.forceSnakeCase);
    ...
    if (shouldIndex(columnName, tablesOptions) && field.type != 'TEXT') {
      ...
      // - Patch: Add a prefix of the table name to the index name to avoid conflicts
      modelOptions.indexes.push({ name: `ix_${modelName}_${sqlName}`, fields: [sqlName] });
